...
--1. consulta 
SELECT
    r.reservation AS codigo_reserva,
    f.flight_number AS numero_vuelo,
    f.service AS fecha_servicio,
    t.ticket_number AS numero_tiquete,
    rp.passenger_sequence_no AS secuencia_pasajero_reserva,
    CONCAT_WS(
        ' ',
        p.first_name,
        p.middle_name,
...
---2. tigger After
CREATE OR REPLACE FUNCTION fn_generate_boarding_pass_after_check_in()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER trg_generate_boarding_pass_after_check_in
AFTER INSERT ON check_in
FOR EACH ROW
EXECUTE FUNCTION fn_generate_boarding_pass_after_check_in();
